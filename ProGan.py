...
class CriticNetwork(nn.Module):
    ...
    def scale_network(self):
        ...
        self.from_rbg = nn.Sequential(nn.Conv2d(3, self.conv_channels[self.scaling_step], kernel_size=1, stride=1, padding=0), 
                                       nn.LeakyReLU(0.2)
                                      )
...
class ProGANModel():
    def __init__(self):
        ...
        self.upscale_interval = [50, 150, 250, 350, 450, 550, 650, 750, 850]
    ...
    def train(self, check_point=None):
        ...
        for epoch in range(epoch_start, self.epochs):
            ...
            for step in range(self.train_steps):            
                ...
                batch_latent =  self.generate_latent(self.batch_size)
    ...
    def generate_batch(self, batch_size):
        ...
        batch = []
        for idx in img_idxs:
            img = plt.imread(self.img_dir + f'{idx:06}' + self.img_ext)
            #img = RandomAffine(180)(img)
            #img = ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.1)(img)
            #img = transforms.RandomHorizontalFlip(p=0.5)(img)
            #img = transforms.RandomVerticalFlip(p=0.5)(img)
            #img = transforms.RandomInvert(p=0.2)
